...
var selectCallbacks = {
    successCallbacks: [function (t) {
        t.$("select").closest('.form-group').removeClass("has-error has-feedback").addClass('has-success has-feedback').delay(2000).queue(function () {
            $(this).removeClass('has-success').dequeue()
        });

        t.$("select").siblings(".yfForm.error-feedback").hide();
        t.$("select").siblings(".yfForm.error-block").hide();
        //t.$("select").siblings(".yfForm.success-feedback").fadeIn(100).delay(1800).fadeOut(100)
    }],
    errorCallbacks: [function (t) {
        t.$("select").closest('.form-group').removeClass("has-success has-feedback").addClass('has-error has-feedback');
        t.$("select").siblings(".yfForm.success-feedback").hide();
        t.$("select").siblings(".yfForm.save-block").hide();
        t.$("select").siblings(".yfForm.error-feedback").show();
        t.$("select").siblings(".yfForm.error-block").show();
    }],
    saveSuccessCallbacks: [function (t) {
        t.$("select").siblings(".yfForm.save-block").fadeIn(300).delay(1800).fadeOut(100);
    }],
    saveErrorCallbacks: [function (t) {
        t.$("select").closest('.form-group').removeClass("has-success has-feedback").addClass('has-error has-feedback');
        t.$("select").siblings(".yfForm.success-feedback").hide();
...
yfSelectField = function (data) {
    this.procData = this.setDefaults(selectCallbacks, data );
    yfBaseField.call(this, data);
}

yfSelectField.prototype = Object.create(yfBaseField.prototype);
yfSelectField.prototype.constructor = yfSelectField;
